...
{
    private readonly ASTGauge gauge = Gauges.Get<ASTGauge>();
    ...
    private uint ReplaceDraw() => MapCard(gauge.DrawnCards[0], MapDraw(gauge.ActiveDraw));
    private uint ReplaceExaltation() => MapCard(gauge.DrawnCards[1], Exaltation);
    private uint ReplaceIntersection() => MapCard(gauge.DrawnCards[2], Intersection);
